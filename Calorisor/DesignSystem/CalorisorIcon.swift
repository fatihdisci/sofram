//
//  CalorisorIcon.swift
//  Calorisor — custom line icons as SwiftUI Shape views.
//
...
enum CalorisorIcon: String, CaseIterable, Identifiable {
    case kepce          // ladle
    case tabak          // plate
    ...
    var primitives: [CalorisorIconPrimitive] {
        switch self {
        case .kepce:
            return [
...
/// A `Shape` built from a `CalorisorIcon`'s primitives, scaled to fill `rect`.
/// Stroke it (don't fill) — these are line icons.
struct CalorisorIconShape: Shape {
...
/// Renders a `CalorisorIcon` at `size` points, stroked with a proportional 1.5px line
/// (relative to the 24pt source). Tint with `.foregroundStyle(...)`.
struct CalorisorIconView: View {
    let icon: CalorisorIcon
    var size: CGFloat = 24

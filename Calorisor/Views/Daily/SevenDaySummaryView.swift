...
//  Calorisor — trailing 7-day calories + bread/tea summary.
//
...
struct SevenDaySummaryView: View {
    ...
    private func dayLabel(for date: Date) -> String {
        ...
        return CalorisorFormatters.turkishShortWeekday.string(from: date).prefix(3).capitalized
    }
}
...
private struct StatCell: View {
    let value: String
    let caption: String
    var icon: CalorisorIcon?

    var body: some View {
        VStack(spacing: 4) {
            HStack(spacing: 4) {
                if let icon {
                    CalorisorIconView(icon: icon, size: 16)
...
struct WeekBarChart: View {
    ...
    private func shortDayLabel(_ date: Date) -> String {
        ...
        return String(CalorisorFormatters.turkishShortWeekday.string(from: date).prefix(3)).capitalized
    }
}

...
//    `Image` or `CalorisorIconView` as `fallback` so the screen still reads
//    correctly pre-asset.
...
//    `Calorisor/Resources/Animations/<name>.json` and add it to the Calorisor
//    target's "Copy Bundle Resources". xcodegen picks up anything under
//    `Calorisor/Resources/` automatically, no extra config needed.
...
struct CalorisorLottieView<Fallback: View>: View {

//
//  ContentView.swift
//  Calorisor — root: onboarding → tabbed home + scan-flow cover.
//
//  v2 structure:
...
//  be split into Calorisor/Views/Settings/ once the project is regenerated.
//
...
struct ContentView: View {
    ...
    @MainActor
    private func tabItemLabel(_ title: String, icon: CalorisorIcon) -> some View {
        Label {
            Text(title)
        } icon: {
    ...
    /// Render the custom Calorisor shape into a template image so the native tab bar
    /// can display it and apply its selected/unselected tint normally.
    @MainActor
    private func tabBarImage(for icon: CalorisorIcon) -> Image {
        let renderer = ImageRenderer(
            content: CalorisorIconView(icon: icon, size: 24)
        ...
        ZStack {
            ...
            VStack(spacing: Layout.Spacing.lg) {
                ...
                ZStack {
                    ...
                    CalorisorIconView(icon: .calorisor, size: 56)
                ...
                Button {
                    ...
                    HStack {
                        Label {
                            ...
                            CalorisorIconView(icon: .calorisor, size: 16)
                                .foregroundStyle(Color.accentFill)
                        }
                        Spacer()

...
//  Calorisor — native StoreKit 2 subscription management.
//
...
final class StoreKitManager {
    ...
    func loadProducts() async {
        ...
        do {
            let products = try await Product.products(for: CalorisorProductID.all)
            for product in products {
                switch product.id {
                case CalorisorProductID.monthly: monthlyProduct = product
                case CalorisorProductID.annual:  annualProduct  = product
                default: break
                }
            }

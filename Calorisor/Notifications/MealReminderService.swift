...
//  Calorisor — user-controlled local notifications (SF-EX06 / EX07 / EX08).
//
...
//
//  Preferences live in UserDefaults under the `calorisor.notif.*` keys so both
...
enum MealSlot: String, CaseIterable, Identifiable {
    ...
    var enabledKey: String { "calorisor.notif.meal.\(rawValue).enabled" }
    var hourKey: String    { "calorisor.notif.meal.\(rawValue).hour" }
    var minuteKey: String  { "calorisor.notif.meal.\(rawValue).minute" }
    var identifierPrefix: String { "calorisor.notif.meal.\(rawValue)" }
}

// MARK: - Preferences (UserDefaults-backed, shared with the settings UI)

enum NotificationPrefs {
    static let masterKey = "calorisor.notif.masterEnabled"

    static let noLogEnabledKey = "calorisor.notif.nolog.enabled"
    static let noLogHourKey    = "calorisor.notif.nolog.hour"
    static let noLogMinuteKey  = "calorisor.notif.nolog.minute"

    static let summaryEnabledKey = "calorisor.notif.summary.enabled"
    static let summaryHourKey    = "calorisor.notif.summary.hour"
    static let summaryMinuteKey  = "calorisor.notif.summary.minute"

    /// One-tap route flag: set when a notification is tapped so the app lands on
    /// the Bugün (daily log) tab.
    static let openDailyKey = "calorisor.notif.openDaily"
        ...
        for dayOffset in 0..<horizonDays {
            ...
            if NotificationPrefs.noLogEnabled {
                ...
                if !alreadyLoggedToday {
                    ...
                    if let fire = fireDate(hour: h, minute: m, on: day, calendar: cal), fire > now {
                        await add(center,
                                  id: "calorisor.notif.nolog.\(dayOffset)",
            ...
            if NotificationPrefs.summaryEnabled {
                ...
                if let fire = fireDate(hour: h, minute: m, on: day, calendar: cal), fire > now {
                    ...
                    await add(center,
                              id: "calorisor.notif.summary.\(dayOffset)",

...
//  Calp — post-capture analysis screen with staggered item reveal.
//
...
struct AnalysisOverlay: View {
                ...
                HStack(spacing: 6) {
                    ForEach(0..<statusCaptions.count, id: \.self) { i in
                        Capsule()
                            ...
                            .animation(.calpSpring, value: statusIndex)
                    }
                }
            }
        }
    ...
    private func startScan() {
        scanTask?.cancel()
        withAnimation(.calpSpring) { scanError = nil }
        statusIndex = 0
...
struct RecognizedItemBadge: View {
    ...
    var body: some View {
        HStack(spacing: Layout.Spacing.sm) {
            ...
            CalpIconView(icon: item.portionUnit.icon ?? .tabak, size: 28)
                .foregroundStyle(.white)

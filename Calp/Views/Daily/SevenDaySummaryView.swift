...
//  Calp — trailing 7-day calories + bread/tea summary.
//
...
struct SevenDaySummaryView: View {
    ...
    private func dayLabel(for date: Date) -> String {
        ...
        return CalpFormatters.turkishShortWeekday.string(from: date).prefix(3).capitalized
    }
}
...
private struct StatCell: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            HStack(spacing: 4) {
                if let icon {
                    CalpIconView(icon: icon, size: 16)
...
struct WeekBarChart: View {
    ...
    private func shortDayLabel(_ date: Date) -> String {
        ...
        return String(CalpFormatters.turkishShortWeekday.string(from: date).prefix(3)).capitalized
    }
}

...
//  Calp — customizable quick-add counters.
//
...
private struct QuickChip: View {
            ...
            ZStack {
                ...
                HStack(spacing: Layout.Spacing.md) {
                    ZStack {
                        ...
                        CalpIconView(icon: item.icon, size: 22)
    ...
    private func increment() {
        ...
        withAnimation(.calpSpring) { onIncrement() }
        spawnGhost("+1")
    }
    ...
    private func decrement() {
        ...
        withAnimation(.calpSpring) { onDecrement() }
        spawnGhost("-1")
    }
...
/// Replaces the previous `easeOut(0.4)` with `Animation.calpBouncy`
/// (Amo95 pattern, retuned for Calp) — a hair of overshoot at the
...
private struct GhostLabel: View {
    ...
    var body: some View {
        Text(text)
            ...
            .onAppear {
                withAnimation(.calpBouncy) { risen = true }
            }
    }
}
...
struct QuickAddEditorView: View {
    ...
    @State private var iconName: String = CalpIcon.tabak.rawValue
                    ...
                    LazyVGrid(columns: iconColumns, spacing: Layout.Spacing.sm) {
                        ForEach(CalpIcon.allCases) { icon in
                            Button {
                                ...
                                CalpIconView(icon: icon, size: 26)
                ...
                HStack(spacing: Layout.Spacing.sm) {
                    ForEach(QuickAddTemplates.all) { template in
                        Button {
                            ...
                            HStack(spacing: 6) {
                                CalpIconView(icon: template.icon, size: 16)
                                    .foregroundStyle(Color.accentFill)

//
//  TextLogView.swift
//  Calp — free-text meal logging alternative.
//
...
struct TextLogView: View {
    ...
    private var analyzeButton: some View {
        Button {
            ...
            HStack(spacing: Layout.Spacing.sm) {
                if isScanning {
                    CalpIconView(icon: .kepce, size: 18)
...
/// Gentle side-to-side wobble — "Calp senin için karıştırıyor" metaphor.
...
final class MealSpeechRecognizer {
    ...
    func start() async {
        guard let recognizer, recognizer.isAvailable else {
            withAnimation(.calpSpring) { state = .unavailable }
            return
        }
        guard await requestSpeechAuthorization() else {
            withAnimation(.calpSpring) { state = .denied(.speech) }
            return
        }
        guard await requestMicrophoneAuthorization() else {
            withAnimation(.calpSpring) { state = .denied(.microphone) }
            return
        }
        ...
        do {
            ...
            withAnimation(.calpSpring) { state = .listening }
        } catch {
            teardownAudio()
            withAnimation(.calpSpring) { state = .failed(error.localizedDescription) }
        }
    }
    ...
    func stop() {
        ...
        task = nil
        request = nil
        withAnimation(.calpSpring) { state = .idle }
    }
    ...
    private func handleRecognitionError() {
        ...
        if transcript.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            task = nil
            request = nil
            withAnimation(.calpSpring) { state = .failed("") }
        } else {
            stop()
        }
    }

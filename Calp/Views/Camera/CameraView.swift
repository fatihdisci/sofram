//
//  CameraView.swift
//  Calp — camera capture screen using AVFoundation.
//
...
//  the app's own Calp chrome (flat page, bordered controls) rather than a
//  full-bleed black takeover — the header, capture button and secondary actions
//  all read as Calp UI, with only the live feed itself inside the card.
//
...
final class CameraManager: NSObject, @unchecked Sendable {
    ...
    private let sessionQueue = DispatchQueue(label: "com.fatih.calp.camera.session")
...
struct CameraView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            if granted {
                camera.startSession()
                withAnimation(.calpSpring.delay(0.15)) { guideVisible = true }
            }
        }
    ...
    private var cardPlaceholder: some View {
        CalpIconView(icon: .tabak, size: 40)
        ...
        VStack(spacing: Layout.Spacing.lg) {
            ...
            Button {
                ...
                ZStack {
                    ...
                    CalpIconView(icon: .capture, size: 30)
    ...
    private func showCaptureError(_ message: String) {
        withAnimation(.calpSpring) { captureErrorMessage = message }
        Task {
            try? await Task.sleep(nanoseconds: 2_200_000_000)
            withAnimation(.calpSpring) { captureErrorMessage = nil }
        }
    }
}

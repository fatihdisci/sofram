...
//  Calp — quick-add counters (customizable) + their per-day tallies.
//
...
final class QuickAddItem {
    ...
    var name: String = ""
    var unit: String = ""
    /// CalpIcon rawValue (e.g. "ekmekDilimi").
    ...
    var icon: CalpIcon { CalpIcon(rawValue: iconName) ?? .tabak }
}
...
enum QuickAddSeed {
    ...
    static func backfillMissingNutrition(_ context: ModelContext) {
        ...
        for item in items {
            ...
            if item.iconName == CalpIcon.tabak.rawValue {

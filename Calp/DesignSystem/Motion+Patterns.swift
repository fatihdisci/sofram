...
//  Calp — iOS 17 native motion patterns ported from
...
//  `Animation.bouncy(duration:extraBounce:)`) so Calp can stay dep-free for
...
//
//  Pattern → Calp use case map:
//  -------------------------------------------------------------------------
//  .calpBouncy       → QuickCounterView "+1"/"-1" ghost rise (WIRED)
//                       Replaces `easeOut(0.4)` with a bouncy spring
//                       matching the spec's "Instagram like-count artışı
//                       hissi, ama abartısız" line.
//  .calpSubtlePulse  → generic "breathing" / "ready" easer, available
//  CalpPulseShine    → PhaseAnimator wrapper for any future "primary
...
//                       milestone badge.
//  CalpStaggerReveal → generic Phase Animator list reveal. NOT wired in
...
extension Animation {
    /// Calp-tuned bouncy spring: iOS 17 `bouncy(duration:extraBounce:)`
    ...
    /// for Calp's 0.5s window.
    ///
    /// Wired: `QuickCounterView.GhostLabel.onAppear`.
    static let calpBouncy = Animation.bouncy(duration: 0.5, extraBounce: 0.15)
    ...
    static let calpSubtlePulse = Animation.easeInOut(duration: 1.8)
}
...
/// self-promoting.
struct CalpPulseShine<Content: View>: View {
    ...
    var body: some View {
        PhaseAnimator([false, true]) { phase in
            ...
            .calpSubtlePulse.repeatForever(autoreverses: true)
        }
    }
}
...
/// results, Calp Modu v1.1 multi-item scans). For a fixed-size 1-5
/// item reveal, the inline `for i in 1...n` loop in `AnalysisOverlay`
/// is more readable.
///
/// Example:
/// ```swift
/// CalpStaggerReveal(items: items, perItem: 0.15) { item in
///     RecognizedItemBadge(item: item, index: 0)
/// }
/// ```
struct CalpStaggerReveal<Item: Identifiable, Content: View>: View {
    let items: [Item]
    let perItem: Double

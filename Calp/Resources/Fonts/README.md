...
1. **Add the font files here** (`Calp/Resources/Fonts/`). Cover the weights the scale uses:
...
2. **Register them in `Info.plist`** by adding a `UIAppFonts` array listing each filename, e.g.:
   ...
   ```
   (Also make sure the files are members of the Calp target so they get bundled. With
   XcodeGen they're picked up automatically once present here; re-run `xcodegen generate`.)

3. **Flip the flag**: set `CalpTypography.geistAvailable = true` in
   `Calp/DesignSystem/Font+Tokens.swift`.

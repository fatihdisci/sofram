//
//  AIProxyClient.swift
//  Calp — networking client for the AI vision proxy.
//
...
enum AIProxyError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .subscriptionRequired:
            return String(localized: "Bu analiz için Calp Pro gerekiyor.")
        case .subscriptionVerificationFailed:
...
private struct OpenAIResponseFormat: Encodable {
    ...
    /// Mirrors `VisionResponse`/`VisionItem` (Calp/Networking/VisionResponse.swift)
...
final class AIProxyClient {
    ...
    private func performProxyRequest(body: AIProxyRequest) async throws -> ScanResult {
        ...
        if let key = configuration.apiKey {
            request.setValue(key, forHTTPHeaderField: "x-calp-key")
        }
        ...
        request.setValue(InstallationIdentity.shared.headerValue,
                         forHTTPHeaderField: "x-calp-installation-id")
        request.setValue(body.appVersion, forHTTPHeaderField: "x-calp-app-version")
        request.setValue("ios", forHTTPHeaderField: "x-calp-platform")
    ...
    private func quotaSnapshot(from response: HTTPURLResponse) -> ScanQuotaSnapshot? {
        guard
            let tier = response.value(forHTTPHeaderField: "x-calp-tier"),
            let photoRemaining = response.value(forHTTPHeaderField: "x-calp-photo-remaining").flatMap(Int.init),
            let photoLimit = response.value(forHTTPHeaderField: "x-calp-photo-limit").flatMap(Int.init),
            let textRemaining = response.value(forHTTPHeaderField: "x-calp-text-remaining").flatMap(Int.init),
            let textLimit = response.value(forHTTPHeaderField: "x-calp-text-limit").flatMap(Int.init)
        else {
            return nil
        }

...
enum SofraIcon: String, CaseIterable, Identifiable {
    case kepce          // ladle
    case tabak          // plate
    ...
    var primitives: [SofraIconPrimitive] {
        switch self {
        case .kepce:
            return [
...
/// A `Shape` built from a `SofraIcon`'s primitives, scaled to fill `rect`.
/// Stroke it (don't fill) — these are line icons.
struct SofraIconShape: Shape {
    let icon: SofraIcon
...
/// Renders a `SofraIcon` at `size` points, stroked with a proportional 1.5px line
/// (relative to the 24pt source). Tint with `.foregroundStyle(...)`.
struct SofraIconView: View {
    let icon: SofraIcon
    var size: CGFloat = 24
...
#Preview("Sofra icons") {
    ...
    return LazyVGrid(columns: columns, spacing: 16) {
        ForEach(SofraIcon.allCases) { icon in
            VStack(spacing: 8) {
                SofraIconView(icon: icon, size: 40)

...
//  CalpTests — end-to-end URLProtocol coverage without network access.
//
...
final class AIProxyClientTests: XCTestCase {
    ...
    func testProxyResponseCarriesServerQuotaHeaders() async throws {
        stub(
            ...
            headers: [
                "x-calp-tier": "free",
                "x-calp-photo-remaining": "1",
                "x-calp-photo-limit": "1",
                "x-calp-text-remaining": "2",
                "x-calp-text-limit": "2",
            ]
        )
    ...
    func testProxyRequestSendsInstallationHeadersAndKeepsIDOutOfBody() async throws {
        ...
        AIClientMockURLProtocol.handler = { request in
            platform = request.value(forHTTPHeaderField: "x-calp-platform")
            installationID = request.value(forHTTPHeaderField: "x-calp-installation-id")
            appVersion = request.value(forHTTPHeaderField: "x-calp-app-version")
            clientKey = request.value(forHTTPHeaderField: "x-calp-key")

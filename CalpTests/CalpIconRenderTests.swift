//
//  CalpIconRenderTests.swift
//  CalpTests — deterministic geometry coverage for the custom icon family.
//
//  These are pure-geometry checks (SwiftUI `Path` math, no running app / no
//  snapshot infra): every `CalpIcon` must have primitives, must render a
...
final class CalpIconRenderTests: XCTestCase {
    ...
    func testEveryIconRendersNonEmptyPath() {
        for icon in CalpIcon.allCases {
            let path = CalpIconShape(icon: icon).path(in: rect)
    ...
    func testEveryIconStaysWithinItsRect() {
        ...
        for icon in CalpIcon.allCases {
            let box = CalpIconShape(icon: icon).path(in: rect).boundingRect
    ...
    func testIconsRenderAcrossUISizesWithoutCollapsing() {
        ...
        for size: CGFloat in [18, 20, 24, 32] {
            ...
            for icon in CalpIcon.allCases {
                let box = CalpIconShape(icon: icon).path(in: square).boundingRect
